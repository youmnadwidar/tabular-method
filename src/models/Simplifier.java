...
public class Simplifier implements ISimplifier {
	...
	public LinkedList<LinkedList<MintermReduced>> simplify(LinkedList<MintermReduced> firststep, int bits,
		...
		minterms = new int[(int) Math.pow(2, bits)];
		fill2Darray(bits);
	...
	public void getRowDominant() {
		...
		while (i > 0) {
			if (getcoverTerms(Reducedterms.get(i)).length > getcoverTerms(Reducedterms.get(i - 1)).length) {
				...
				while (i > 0
						&& getcoverTerms(Reducedterms.get(i)).length == getcoverTerms(Reducedterms.get(i - 1)).length) {
	...
	public void getEssential(int[] wantedTerms) {
		LinkedList<Integer> essentials = new LinkedList<>();;
		int j = 0;
